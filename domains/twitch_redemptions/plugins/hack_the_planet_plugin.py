...
class HackThePlanetPlugin(BasePlugin):
    ...
    def __init__(self, twitch, http_client, logger):
        self.twitch = twitch
    ...
    async def _on_redemption(self, event: dict):
        ...
        if not self.webhook_url:
            self.logger.warning("[HackThePlanet] HOME_ASSISTANT_WEBHOOK not set — skipping.")
            return

        self.logger.info(f"[HackThePlanet] Reward '{title}' redeemed by {event.get('user_name')}. Triggering HA webhook...")
        ...
        try:
            resp = await self.http_client.post(self.webhook_url, json=payload, timeout=5.0)
            if resp["ok"]:
                self.logger.info("[HackThePlanet] HA Webhook triggered successfully.")
            else:
                self.logger.error(f"[HackThePlanet] HA Webhook failed with status {resp['status']}: {resp['text']}")
